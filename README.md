...
│── detector.py         # People detection logic
...
1. Capture video input using a webcam
2. Use HOG (Histogram of Oriented Gradients) with SVM for human detection
3. Draw bounding boxes around detected individuals
4. Count the number of detected people
5. Classify crowd density:

   * Low (0–2 people)
   * Medium (3–6 people)
   * High (7+ people)
6. Display results on screen with alerts
...
2. For each frame:
   a. Detect humans using HOG + SVM
   b. Count number of detections
   c. Classify density level
   d. Display count and alert
3. Repeat until exit
...
bash
git clone https://github.com/your-username/crowd-density-system.git
cd crowd-density-system
